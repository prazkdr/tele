...
exports.start = async(lol, name) => {
    text = `Halo ${name}! Selamat Datang di Martin Bot Telegram. ❤️ by  [@martinmagang].`
    await lol.replyWithMarkdown(text, { disable_web_page_preview: true })
}

exports.help = async(lol, name) => {
    text = `Halo ${name}! Selamat Datang di Martin Bot Telegram`
    options = {
        reply_markup: {
            inline_keyboard: [
                [
                    { text: 'Islami ☪️', callback_data: 'islami' },
                    { text: 'Download 📥', callback_data: 'downloader' }
                ],
                [
...
exports.messageError = async(lol) => {
    await lol.reply(`Jika kamu menemukan Error, mohon menghubungi! [${config.owner}](${config.ownerLink}) Untuk melaporkannya.`, { parse_mode: "Markdown", disable_web_page_preview: true })
}