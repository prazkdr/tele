...
if (bot_token == "") {
    return console.log("1769653608:AAGvlrlBkf8vlfIiKwDSHHecZNthcrzeW3k")
}
    ...
    try {
        ...
        switch (command) {
            ...
            case 'kisahnabi':
                ...
                break
            case 'jadwalsholat':
                if (args.length == 0) return reply(`Example: ${prefix + command} Kediri`)
                daerah = args.join(" ")
                ...
                // Downloader //
            case 'ytplay':
                if (args.length == 0) return await reply(`Example: ${prefix + command} to the bone`)
                ...
                break
            case 'ytsearch':
                if (args.length == 0) return reply(`Example: ${prefix + command} to the bone`)
                try {
                    query = args.join(" ")
            ...
            case 'ytmp4':
                ...
                caption += `\`❖ Size     :\` *${result.link[3].size}*`
                await lol.replyWithPhoto({ url: result.thumbnail }, { caption: caption, parse_mode: "Markdown" })
                if (Number(result.link[0].size.split(` MB`)[0]) >= 50.00) return reply(`Maaf ukuran file tidak boleh lebih dari 50 MB!`)
            ...
            case 'horrorblood':
            case 'thunder':
                if (args.length == 0) return reply(`Example: ${prefix + command} Martin Magang`)
                text = args.join(" ")
            ...
            default:
                if /*(!isGroup &&*/ (!isCmd) {
                    ...
                    simi = await fetchJson(`https://api.simsimi.net/v1/?text=${body}&lang=id`)
